...
export class LLMService {
     ...
     * 使用LLM判断新闻是否为科技类
     */
    async analyzeTechNews(title, description) {
        try {
...
请以JSON格式回复，包含以下字段：
{
  ...
  "reasoning": string // 判断理由
    ...
    fallbackAnalysis(title, description) {
        ...
        const foundKeywords = techKeywords.filter(keyword => content.includes(keyword.toLowerCase()));
    ...
    async batchAnalyze(newsItems) {
        ...
        for (let i = 0; i < newsItems.length; i += batchSize) {
            ...
            const batchPromises = batch.map(item => this.analyzeTechNews(item.title, item.description));
     ...
     * 计算新闻相关性评分
     */
    calculateRelevanceScore(newsItem, analysis) {
        let score = 50; // 基础分
        // 科技类新闻加分
