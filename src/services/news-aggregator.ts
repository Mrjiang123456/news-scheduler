...
import { truncateText, formatTime } from '../utils/helpers.js';
import { logger } from '../utils/logger.js';
import { LLMService, LLMResponse } from './llm-service.js';
...
export class NewsAggregator {
   ...
   * 生成新闻摘要（集成LLM智能分析）
   */
  async generateDigest(news: NewsItem[]): Promise<NewsDigest> {
   ...
   * 按相关性排序新闻（优先展示科技类新闻）
   */
  private sortNewsByRelevance(news: NewsItem[]): NewsItem[] {
   ...
   * 计算包含科技类权重的最终评分
   */
  private calculateFinalScore(item: NewsItem): number {
   ...
   * 统计新闻分类
   */
  private categorizeNews(news: NewsItem[]): Record<string, number> {
   ...
   * 生成摘要文本（突出科技类新闻）
   */
  private generateSummaryText(news: NewsItem[], categories: Record<string, number>): string {
    if (news.length === 0) {
      return '今日暂无新闻更新。';
    }
   ...
   * 提取科技新闻的关键词
   */
  private extractTechKeywords(techNews: NewsItem[]): string[] {
   ...
   * 提取热门话题
   */
  private extractHotTopics(news: NewsItem[]): string[] {
   ...
   * 使用LLM增强新闻分析
   */
  private async enhanceNewsWithLLM(news: NewsItem[]): Promise<NewsItem[]> {
    try {
      // 批量处理新闻，每批最多10条
      const batchSize = 10;
   ...
   * 计算包含LLM评分的增强评分
   */
  private calculateEnhancedScore(item: NewsItem, llmResponse?: LLMResponse): number {
    let score = item.score || 0;
   ...
   * 使用LLM生成增强摘要文本
   */
  private async generateSummaryTextWithLLM(news: NewsItem[], categories: Record<string, number>): Promise<string> {
    try {
      // 先生成基础摘要
   ...
   * 去重新闻（基于内容相似度）
   */
  deduplicateNews(news: NewsItem[], similarityThreshold: number = 0.8): NewsItem[] {
   ...
   * 过滤低质量新闻
   */
  filterQualityNews(news: NewsItem[], minScore: number = 30): NewsItem[] {
    const filtered = news.filter(item => {
      ...
      // 标题质量检查
      if (this.isLowQualityTitle(item.title)) return false;
   ...
   * 检查是否为低质量标题
   */
  private isLowQualityTitle(title: string): boolean {
